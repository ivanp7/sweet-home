...
def widget_exit_code(style: dict, code: int) -> str:
    """Exit code widget.
    """
    if code is None:
        return "", 0
...
def widget_exec_time(style: dict, seconds: int) -> str:
    ...
    return p_fg(style.get('fg')) + text, len(text)
...
if __name__ == '__main__':
    ...
    if prompt_type == "cmd_result":
        ...
        if len(sys.argv) > arg_index:
            ...
            exit_code = int(sys.argv[arg_index])
        ...
        if len(sys.argv) > arg_index:
            ...
            exec_time = int(sys.argv[arg_index])
        ...
        prompt_string += p_bg(bg, None, chr_triangle_left, False) + \
                p_fg(fg) + status + p_bg(None, bg, chr_triangle_right, True) + chr_bracket_right + " "
        prompt_length += len(chr_triangle_left) + len(status) + len(chr_triangle_right) + len(chr_bracket_right) + 1
        ...
        prompt_length += len(chr_bracket_left) + len(chr_triangle_left) + len(status) + len(chr_triangle_right) + 1
